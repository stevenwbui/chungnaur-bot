...
testing = bot.create_group('testing', 'For testing the bot')

@testing.command(guild_ids=[979520985365639238], name='hello', description='wishes this user')
async def hello(ctx,
    user: Option(discord.Member, 'The user to wish',
            required = False, default = None)):

    if user == None:
        user = ctx.author

    await ctx.respond(f'{ctx.author} wished {user}!')
...
async def menu(ctx):
    ...
    class gachaView(View):
        @discord.ui.button(style=discord.ButtonStyle.gray, label='150 | Normal', emoji=bot.get_emoji(992242623127490630))
